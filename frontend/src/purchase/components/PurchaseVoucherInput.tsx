...
import { useGetSuppliers } from '../../supplier/hooks/api.hooks';
...
import { SupplierChooserPopupProps, PurchaseVoucherInputProps } from '../types/purchase.types';

function SupplierChooserPopup({ companyId, onSelect, onClose }: SupplierChooserPopupProps) {
  const { data: suppliers = [], isLoading, isError } = useGetSuppliers(companyId);
  const [query, setQuery] = useState('');
  const [focusedSupplier, setFocusedSupplier] = useState<Supplier | null>(null);
  ...
  if (isError) {
    return (
      ...
      >
        <div className="p-4">
          <p className="text-sm text-red-600 font-semibold">Could not load suppliers.</p>
        </div>
      </Popup>
    );
  }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', height: '350px' }}>
        ...
        <div style={{ flex: 1, minHeight: 0 }}>
          <List<Supplier>
            ...
            onFocusedItemChanged={setFocusedSupplier}
            renderItem={(supplier) => (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', whiteSpace: 'normal', wordBreak: 'break-word' }}>
                <span style={{ fontSize: '0.75rem', color: '#64748b' }}>{supplier.code}</span>
                <span style={{ fontSize: '0.875rem', fontWeight: 500 }}>{supplier.name}</span>
              </div>
            )}
          />
        </div>
...
export default function PurchaseVoucherInput({ onSave, initialData, serverErrors, voucherNumber }: PurchaseVoucherInputProps) {
  ...
  const [errors, setErrors] = useState<{ supplier?: string; voucherDate?: string }>({});
  ...
  }, [supplier, voucherDate, onSave, showToast]);
  ...
  useEffect(() => {
    registerShortcuts("PurchaseVoucherLeft", leftShortcuts);
    return () => { unregisterShortcuts("PurchaseVoucherLeft"); };
  ...
  useEffect(() => {
    ...
    registerShortcuts('PurchaseVoucherInput', inputShortcuts);
    return () => { unregisterShortcuts('PurchaseVoucherInput'); };
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="smarterp-box">
              <div className="smarterp-header">
                <span className="smarterp-title">
                  {voucherNumber ? 'Edit Purchase Voucher' : 'New Purchase Voucher'}
                </span>
                {voucherNumber && (
              ...
              {/* Single row layout with Date first and Supplier second */}
              <div style={{ display: 'flex', flexDirection: 'row', gap: '2rem', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
                ...
                <div style={{ flex: 1 }}>
                  <ErpChooser<Supplier>
                    label="Supplier"
                    value={supplier ? `${supplier.name}${supplier.code ? ` (${supplier.code})` : ''}` : ''}
                    required
