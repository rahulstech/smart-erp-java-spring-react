...
import { useGetPurchaseVoucherById } from '../hooks/api.hooks';
...
const COLUMNS = ['Stock Item', 'Quantity', 'Unit Price', 'Amount'];

export default function PurchaseVoucherItemsList() {
  ...
  const leftShortcuts = useMemo<KeyboardShortcut[]>(() => [
    ...
    },
    {
      ...
      label: 'Manage Items',
      handler: () => {
        if (company_id && voucher_id && APP_ROUTES.ADD_PURCHASE_ITEM.create) {
          navigate(APP_ROUTES.ADD_PURCHASE_ITEM.create(company_id, voucher_id));
        }
      }
    }
  ...
  useEffect(() => {
    registerShortcuts("PurchaseItemsLeft", leftShortcuts);
    return () => {
      unregisterShortcuts("PurchaseItemsLeft");
    };
            ...
            <>
              ...
              <div className="p-4 bg-zinc-50 border-b border-zinc-200">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    <span className="block text-[10px] uppercase tracking-wider text-zinc-500 font-semibold">Supplier</span>
                    <span className="text-sm font-bold text-zinc-800">{voucher.supplierName}</span>
                  </div>
                  <div>
              ...
              <div className="flex-1 min-h-0 relative flex flex-col">
                <ErpTable
                  ...
                  render={(colIndex, item) => {
                    if (colIndex === 0) {
                      return <span className="font-semibold">{item.stockItemName || 'N/A'}</span>;
                    }
                    if (colIndex === 1) {
                      return <span className="font-mono">{item.quantity}</span>;
                    }
                    if (colIndex === 2) {
                      return <span className="font-mono">₹ {Number(item.unitPrice || 0).toFixed(2)}</span>;
                    }
                    if (colIndex === 3) {
                      return (
                  ...
                  onFilter={(item, query) => {
                    ...
                    if (!q) return true;
                    return (item.stockItemName || '').toLowerCase().includes(q);
                  }}
                />
              </div>
            </>
