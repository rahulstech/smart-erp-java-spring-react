...
import { useGetSaleVoucherById, useUpdateSaleVoucherItems } from '../hooks/api.hooks';
...
import { ItemRow } from '../types/sale.types';

export default function AddSaleItemPage() {
  ...
  const updateItemsMutation = useUpdateSaleVoucherItems(company_id || '', voucher_id || '');

  const [rows, setRows] = useState<ItemRow[]>([]);
  const [rowErrors, setRowErrors] = useState<Record<string, { stockItem?: string; quantity?: string; rate?: string }>>({});
  ...
  useEffect(() => {
    if (voucher) {
      if (voucher.items && voucher.items.length > 0) {
        ...
        setRows([{ key: Math.random().toString(), stockItem: null, hsnCode: '', quantity: '1', rate: '0' }]);
      }
    }
  }, [voucher, company_id]);
  ...
  const handleAddRow = () => {
    setRows(prev => [...prev, { key: Math.random().toString(), stockItem: null, hsnCode: '', quantity: '1', rate: '0' }]);
  };
  ...
  const handleRemoveRow = (key: string) => {
    setRows(prev => {
      ...
      if (filtered.length === 0) {
        return [{ key: Math.random().toString(), stockItem: null, hsnCode: '', quantity: '1', rate: '0' }];
      }
      return filtered;
    });
    setRowErrors(prev => {
  ...
  const handleSave = useCallback(() => {
    ...
    rows.forEach(row => {
      const errors: { stockItem?: string; quantity?: string; rate?: string } = {};
      if (!row.stockItem) {
      ...
      if (!row.rate.trim() || isNaN(rateNum) || rateNum < 0) {
    ...
    updateItemsMutation.mutate(payload, {
      onSuccess: () => {
        ...
        if (company_id && voucher_id && APP_ROUTES.SALE_VOUCHER_ITEMS_LIST.create) {
          navigate(APP_ROUTES.SALE_VOUCHER_ITEMS_LIST.create(company_id, voucher_id));
        } else {
          navigate(-1);
        }
      },
  ...
  useEffect(() => {
    registerShortcuts("AddSaleItemLeft", leftShortcuts);
    return () => { unregisterShortcuts("AddSaleItemLeft"); };
  ...
  useEffect(() => {
    ...
    registerShortcuts('AddSaleItemActions', actionShortcuts);
    return () => { unregisterShortcuts('AddSaleItemActions'); };
  ...
  return (
    <div className="flex w-full h-full overflow-hidden">
        ...
        ? (
            <div className="flex w-full h-full justify-center items-center p-6">
              <Card className="max-w-md w-full mx-auto">
                ...
                <p className="text-xs text-zinc-500 mt-2"><strong>F5</strong>: Retry</p>
              </Card>
            </div>
        )
        : (
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-1">
                    Voucher: <strong className="font-mono text-zinc-700">{voucher.voucherNumber}</strong> | Customer: <strong>{voucher.customerName}</strong>
                  </p>
                </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-zinc-200 text-xs uppercase tracking-wider text-zinc-500">
                      <th className="pb-2 w-2/5">Stock Item</th>
                      <th className="pb-2 px-2 w-1/5">HSN Code</th>
                      <th className="pb-2 px-2 w-1/10">Quantity</th>
                      <th className="pb-2 px-2 w-1/10">Rate</th>
                      <th className="pb-2 px-2 w-1/10">Amount (₹)</th>
                      ...
                      return (
                        <tr key={row.key} className="border-b border-zinc-100 last:border-0 align-top">
                          ...
                          <td className="py-3 px-2">
                            <input
                              type="text"
                              className="smarterp-input"
                              value={row.hsnCode}
                              onChange={(e) => handleRowChange(row.key, 'hsnCode', e.target.value)}
                          ...
                          <td className="py-3 px-2">
                            <input
                              type="number"
                              className={`smarterp-input ${rowErrors[row.key]?.rate ? 'has-error' : ''}`}
                              value={row.rate}
                              onChange={(e) => handleRowChange(row.key, 'rate', e.target.value)}
                              placeholder="Rate"
                              min="0"
                              step="0.01"
                            />
                            {rowErrors[row.key]?.rate && (
                              <span className="smarterp-field-error block mt-1">{rowErrors[row.key].rate}</span>
                            )}
                          </td>
      ...
      {updateItemsMutation.isPending && <LoadingPopup message="Saving items and updating stock..." />}
