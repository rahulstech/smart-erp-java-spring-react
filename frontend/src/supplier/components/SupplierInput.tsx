...
import { SupplierInputProps } from '../types/supplier.types';
...
/**
 * Renders the form interface to create or edit a Supplier's details.
...
export default function SupplierInput({ onSave, initialData, serverErrors, isEditing }: SupplierInputProps) {
  ...
  const [formData, setFormData] = useState<FormState>(() => {
    if (initialData) {
      return {
        ...
        openingBalance: (initialData.openingBalance ?? 0).toString()
      };
    }
  ...
  }, [formData, isEditing, onSave, showToast]);
  ...
  useEffect(() => {
    ...
    registerShortcuts('SupplierInput', shortcuts);
    return () => { unregisterShortcuts('SupplierInput'); };
